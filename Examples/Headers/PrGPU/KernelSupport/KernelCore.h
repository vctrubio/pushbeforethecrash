...
#if VULKAN
#   define GF_DEVICE_TARGET_VULKAN 1
/**
**	The Vulkan support here uses a variation of the OpenCL C flavor of kernel pre-processing,
**	compiling that code to the graphics flavor SPIR-V consumed by Vulkan compute pipelines.
*/
#   define GF_DEVICE_TARGET_OPENCL 1
#else
#   define GF_DEVICE_TARGET_VULKAN 0
#endif
...
#	define GF_GET_CONSTANT(inDeclarator) inDeclarator
#	define GF_CONSTANT_PTR(Type) const GF_PTR(Type)
...
#	else //host
#		define GF_NONCUDA_SHARED_DYNAMIC(Type, Name) 	GF::ArgumentFramework<GF::kArgumentFrameworkSharedMemory, GF_SHARED(Type*)>
#	endif
...
#	define GF_TEXTURE_GLOBAL_3D(PixelType, Name, inDomain, inRange, inEdge, inFilter) static texture<PixelType, cudaTextureType3D, inRange> Name;
...
#	define GF_READTEXTURE3D(Name, inX, inY, inZ) read_imagef(Name, kSampler_##Name, (float4)((inX), (inY), (inZ), (1.0f)))
...
template<typename ChannelType> using GFMetal3DTextureType = texture3d<ChannelType, access::sample>;
#	define GF_TEXTURE_TYPE(ChannelType) GFMetalTextureType<ChannelType> //boost preprocessor sequences don't like commas
#	define GF_TEXTURE_TYPE_3D(ChannelType) GFMetal3DTextureType<ChannelType> //boost preprocessor sequences don't like commas
#	define GF_TEXTURE_NAME(Name) Name
#	define GF_GET_TEXTURE(Name) Name
